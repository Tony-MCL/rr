...
func _run_test() -> void:
	...
	var a_total: int = _hit_construction(construction_a_target, 6)
	...
	var b_total: int = _hit_construction(construction_b_target, 6)
	...
	var c_total: int = _hit_construction(construction_c_target, 6)
	...
	var d_total: int = _hit_construction(construction_d_target, 6)
	...
	var restart_total: int = _hit_construction(construction_a_target, 6)
	_check(restart_total == 310, "Explicit break should restart the super-chain at x1.", failures)

	var before_return: int = int(score_controller.get_score())
	_hit_construction(construction_b_target, 6)
	var return_start: int = int(score_controller.get_score())
	score_controller.register_construction_hit(cannonball, construction_a_target)
	var return_points: int = int(score_controller.get_score()) - return_start
	...
	_check(int(score_controller.get_score()) > before_return, "Return test should continue producing construction score.", failures)
...
func _hit_construction(target: TargetBody, hit_count: int) -> int:
	var start_score: int = int(score_controller.get_score())
	for index in range(hit_count):
		score_controller.register_construction_hit(cannonball, target)
	return int(score_controller.get_score()) - start_score
